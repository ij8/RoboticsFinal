...
class MyController:
	...
	robot = self.world.robot(0)
	    ...
	    self.qdes = [0,1.3,-1.5,.9,0,-.9,1.5]
	    self.dqdes = [0,0,0,0,0,0,0]
	    dt = .7
	    robotController.setMilestone([0.0]*7,self.dqdes)
	    robotController.setCubic(self.qdes,self.dqdes,dt)
	    count = 0
	    for i in range(0,len(qcmd)):
		if round(qcmd[i]*10**6) == round(qsns[i]*10**6):
		    count += 1
	    if count == len(qcmd):
	        self.state = 'sensing'
	elif self.state == 'sensing':
	    """
		The following code uses the blob detector 
		to check and see if walls are clear from 
		the target region
	    """
	    wallCount = 0
	    clearCount = 0
	    for obj in objectStateEstimate.objects:
		if obj.meanPosition()[2] > .2:
		    wallCount += 1		
		    if obj.meanPosition()[1] > 0 or obj.meanPosition()[1] < -1:
			clearCount += 1
	    if clearCount == wallCount:
		self.state = 'strike' 
	elif self.state == 'strike':
	    # Motion Queue Method for Striking
	    self.qdes = [0,2,-1.5,.9,0,-.9,1.5]
	    self.dqdes = [0,0,0,0,0,0,0]
	    dt = .2
	    robotController.setMilestone([0.0]*7,self.dqdes)
	    robotController.setCubic(self.qdes,self.dqdes,dt)
